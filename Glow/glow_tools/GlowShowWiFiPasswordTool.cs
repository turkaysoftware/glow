    ...
    public partial class GlowShowWiFiPasswordTool : Form{
        ...
        public void swpt_theme_settings(){
            try{
                ...
                Text = string.Format(Encoding.UTF8.GetString(Encoding.Default.GetBytes(software_lang.TSReadLangs("ShowWiFiPasswordTool", "swpt_title").Trim())), Application.ProductName);
                //
                SWPT_TitleLabel.Text = Encoding.UTF8.GetString(Encoding.Default.GetBytes(software_lang.TSReadLangs("ShowWiFiPasswordTool", "swpt_in_title").Trim()));
                SWPT_CopyBtn.Text = Encoding.UTF8.GetString(Encoding.Default.GetBytes(software_lang.TSReadLangs("ShowWiFiPasswordTool", "swpt_copy_btn").Trim()));
            }catch (Exception){ }
        }
        // LOAD SWPT
        // ======================================================================================================
        private void GlowShowWiFiPasswordTool_Load(object sender, EventArgs e){
            ...
            try{
                ...
                if (wifi_profileNames.Length == 0){
                    MessageBox.Show(string.Format(Encoding.UTF8.GetString(Encoding.Default.GetBytes(software_lang.TSReadLangs("ShowWiFiPasswordTool", "swpt_no_profile").Trim())), "\n", Encoding.UTF8.GetString(Encoding.Default.GetBytes(software_lang.TSReadLangs("HeaderTools", "ht_show_wifi_password_tool").Trim()))), Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    Close();
                }
        ...
        private void SWPT_SelectBox_SelectedIndexChanged(object sender, EventArgs e){
            try{
                ...
                if (startIndex != -1){
                    ...
                    TSGetLangs software_lang = new TSGetLangs(Glow.lang_path);
                    SWPT_ResultBox.Text = string.Format(Encoding.UTF8.GetString(Encoding.Default.GetBytes(software_lang.TSReadLangs("ShowWiFiPasswordTool", "swpt_select_profile_no_password").Trim())), networkName);
                }
            }catch (Exception){ }
        }
        ...
        private void SWPT_CopyBtn_Click(object sender, EventArgs e){
            try{
                ...
                TSGetLangs software_lang = new TSGetLangs(Glow.lang_path);
                MessageBox.Show(Encoding.UTF8.GetString(Encoding.Default.GetBytes(software_lang.TSReadLangs("ShowWiFiPasswordTool", "swpt_copy_txt").Trim())), Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
            }catch (Exception){ }
        }
    }
}