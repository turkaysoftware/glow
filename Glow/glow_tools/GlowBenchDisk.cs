    ...
    public partial class GlowBenchDisk : Form{
        ...
        public void bench_disk_theme_settings(){
            try{
                ...
                //
                Bench_L_WriteSpeed.Text = Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bd_time_write").Trim()));
                Bench_L_Max_WriteSpeed.Text = Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bd_time_max_write").Trim()));
                Bench_R_ReadSpeed.Text = Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bd_time_read").Trim()));
                Bench_R_Max_ReadSpeed.Text = Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bd_time_max_read").Trim()));
                //
                Bench_Size.Items[7] = (Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bd_test_size_custom").Trim())));
                //
        ...
        private void GlowBenchDisk_Load(object sender, EventArgs e){
            ...
            //
            Bench_L_WriteSpeed_V.Text = Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bd_start_test_await").Trim()));
            Bench_L_Max_WriteSpeed_V.Text = Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bd_start_test_await").Trim()));
            Bench_R_ReadSpeed_V.Text = Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bd_start_test_await").Trim()));
            Bench_R_Max_ReadSpeed_V.Text = Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bd_start_test_await").Trim())); 
            //
                ...
                if (Bench_Size.SelectedIndex == Bench_Size.Items.Count - 1){
                    if (Bench_SizeCustom.Text.Trim() != "" || Bench_SizeCustom.Text.Trim() != string.Empty){
                        if (Convert.ToDouble(Bench_SizeCustom.Text.Trim()) >= 10 && Convert.ToDouble(Bench_SizeCustom.Text.Trim()) <= 256){
                            ...
                            MessageBox.Show(Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bd_space_req").Trim())), Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void BenchTimer(){
            ...
            try{
                do{
                    ...
                    TSGetLangs g_lang = new TSGetLangs(Glow.lang_path);
                    global_timer = Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bt_elapsed_time").Trim())) + " " + string.Format("{0:D2}:{1:D2}:{2:D2}", fh_hour, fh_minute, fh_second);
                    Thread.Sleep(1000);
                } while (loop_mode);
            }catch (Exception){ }
        }
        ...
        private void UpdateProgress(double progress){
            TSGetLangs g_lang = new TSGetLangs(Glow.lang_path);
            Text = string.Format(Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bd_title").Trim())), Application.ProductName) + " - " + global_timer + " - " + progress.ToString("0.00") + "%";
        }
        // DISK BENCHMARK
        ...
        private void stop_engine(){
            ...
            if (File.Exists(benchmarkFilePath)){
                File.Delete(benchmarkFilePath);
                Text = string.Format(Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bd_title").Trim())), Application.ProductName);
                MessageBox.Show(Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("BenchDisk", "bd_result_exit").Trim())), Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        // CUSTOM SIZE CHANGE
