    ...
    public partial class GlowMonitorTestEngine : Form {
        ...
        private void monitor_test_loader(int monitor_test_mode){
            if (monitor_test_mode == 0){
                InfoLabel.Text = string.Format(Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("MonitorTestTool", "mtt_esc_info_dead_pixel").Trim())), "\n");
                monitor_test_dead_pixel();
            }else if (monitor_test_mode == 1){
                InfoLabel.Text = Encoding.UTF8.GetString(Encoding.Default.GetBytes(g_lang.TSReadLangs("MonitorTestTool", "mtt_esc_info_dynamic_range").Trim()));
