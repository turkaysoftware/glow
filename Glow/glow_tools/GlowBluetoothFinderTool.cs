    ...
    public partial class GlowBluetoothFinderTool : Form{
        ...
        public void BTFinder_Preloader(){
            try{
                ...
                BackColor = TS_ThemeEngine.ColorMode(Glow.theme, "ContentPanelBGColor");
                //
                BTSelect.BackColor = TS_ThemeEngine.ColorMode(Glow.theme, "SelectBoxBGColor");
                BTSelect.ForeColor = TS_ThemeEngine.ColorMode(Glow.theme, "AccentMain");
                //
                foreach (Control ui_buttons in BackPanel.Controls){
                    if (ui_buttons is Button bt_finder_btn){
                        bt_finder_btn.ForeColor = TS_ThemeEngine.ColorMode(Glow.theme, "DynamicThemeActiveBtnBG");
                        bt_finder_btn.BackColor = TS_ThemeEngine.ColorMode(Glow.theme, "AccentMain");
                        bt_finder_btn.FlatAppearance.BorderColor = TS_ThemeEngine.ColorMode(Glow.theme, "AccentMain");
                        bt_finder_btn.FlatAppearance.MouseDownBackColor = TS_ThemeEngine.ColorMode(Glow.theme, "AccentMain");
                        bt_finder_btn.FlatAppearance.MouseOverBackColor = TS_ThemeEngine.ColorMode(Glow.theme, "AccentMainHover");
                    }
                }
                //
                foreach (Control ui_panels in BackPanel.Controls){
                    if (ui_panels is Panel bt_finder_panel){
                        bt_finder_panel.BackColor = TS_ThemeEngine.ColorMode(Glow.theme, "PageContainerBGAndPageContentTotalColors");
                    }
                }
                //
                ...
                left_labels.ToList().ForEach(l => l.ForeColor = TS_ThemeEngine.ColorMode(Glow.theme, "ContentLabelLeft"));
                right_labels.ToList().ForEach(l => l.ForeColor = TS_ThemeEngine.ColorMode(Glow.theme, "AccentMain"));
                //
                TSImageRenderer(BTCopyInfoBtn, Glow.theme == 1 ? Properties.Resources.ct_copy_mc_light : Properties.Resources.ct_copy_mc_dark, 15, ContentAlignment.MiddleRight);
                //
                // ======================================================================================================
                // TEXTS
                TSGetLangs software_lang = new TSGetLangs(Glow.lang_path);
        ...
        private async void LoadBluetoothAdapters(){
            ...
            foreach ($device in $bluetoothDevices) {
                ...
                # Son olarak donanım kimliğini ekle
        ...
        private void GlowBluetoothFinderTool_FormClosing(object sender, FormClosingEventArgs e){
            if (process_running){
                TSGetLangs software_lang = new TSGetLangs(Glow.lang_path);
                TS_MessageBoxEngine.TS_MessageBox(this, 1, software_lang.TSReadLangs("BluetoothFinderTool", "bft_exit"));
                e.Cancel = true;
            }
        }
    }
