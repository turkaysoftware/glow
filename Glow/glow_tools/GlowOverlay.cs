...
namespace Glow.glow_tools{
    public partial class GlowOverlay : Form{
        ...
        private void GlowOverlay_Load(object sender, EventArgs e){
            OVERLAY_CPU_V.Text = software_lang.TSReadLangs("ScreenOverlayTool", "sot_loading");
            OVERLAY_RAM_V.Text = software_lang.TSReadLangs("ScreenOverlayTool", "sot_loading");
            OVERLAY_DISK_V.Text = software_lang.TSReadLangs("ScreenOverlayTool", "sot_loading");
            OVERLAY_NETWORK_V.Text = software_lang.TSReadLangs("ScreenOverlayTool", "sot_loading");
            // LOAD
        ...
        public void Screen_overlay_settings(){
            // TSSetWindowTheme(Handle, Glow.theme);
            //
            ...
            try{
                do{
                    ...
                    if (!cDiskEntry.Equals(default(KeyValuePair<string, (ulong readSpeed, ulong writeSpeed)>))){
                        ...
                        float cDiskReadSpeedMB = readSpeed / (1024 * 1024);
                        float cDiskWriteSpeedMB = writeSpeed / (1024 * 1024);
                    ...
                    if (!dDiskEntry.Equals(default(KeyValuePair<string, (ulong readSpeed, ulong writeSpeed)>))){
                        ...
                        float dDiskReadSpeedMB = readSpeed / (1024 * 1024);
                        float dDiskWriteSpeedMB = writeSpeed / (1024 * 1024);
                ...
                do{
                    ...
                    foreach (ManagementObject search_active_speed in searcher_net_speed.Get().Cast<ManagementObject>()){
                        if (Net_replacer(search_active_speed["Name"].ToString().Trim()) == activeAdapter){
                            ...
                            OVERLAY_NETWORK_V.Text = string.Format("{0}\n{1}", string.Format("{0:0.00} Mbps", software_lang.TSReadLangs("ScreenOverlayTool", "sot_net_download") + " " + mbpsReceived), string.Format("{0:0.00} Mbps", software_lang.TSReadLangs("ScreenOverlayTool", "sot_net_upload") + " " + mbpsSent));
                        }
                    }
                    Thread.Sleep(750);
                }while (overlay_loop == true);
